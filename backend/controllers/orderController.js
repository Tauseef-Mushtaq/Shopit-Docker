...
export const updateOrder = catchAsyncErrors(async (req, res, next) => {
  ...
  if (order?.orderStatus === 'Delivered') {
    return next(new ErrorHandler('You have alerady delivered this order', 400));
  }
...
async function getSalesData(startDate, endDate) {
  ...
  //Create a map to store sale and num of orders by date
  ...
  const finalSalesDates = datesBetween.map((date) => ({
    date,
    sales: (salesMap.get(date) || { sales: 0 }).sales,
    numOrders: (salesMap.get(date) || { numOrders: 0 }).numOrders,
  }));
  return { salesData: finalSalesDates, totalSales, totalNumOrders };
}
